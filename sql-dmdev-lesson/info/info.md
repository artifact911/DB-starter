...
20. [View](#view)
21. [Alter](#alter)
22. [Union. Intersect. Except](#union-intersect-except)
...
[Оглавление](#start)
## <p id='alter'>25. Alter</p>
В доке БД нужно почитать, что можно делать при помощи ALTER. Во многих СУБД сильные отличия
1. Добавить колонку

        ALTER TABLE IF EXISTS employee
        ADD COLUMN gender INT;

    Мы не можем сейчас установить NOT NULL для поля, т.к. что делать с теми, что там сейчас есть? 
    Значит, сначала устанавливаем знаечения, а затем устанавливаем constraints
...
[Оглавление](#start)
## <p id='union-intersect-except'>27. Union. Intersect. Except</p>
У нас есть 2 выборки: select1 и select2

1. Union All
   - просто объеденит 2 выборки

2. Union:
   - объеденит 2 выборки, но при наличие дубликатов не будет из дублировать, в выборке будет только один вариант
   
3. Intersect:
   - Покажет только те значения, которые встречаются в одной и другой выборке
   
4. Except:
   - Покажет одну выборку исключив то, что есть во второй